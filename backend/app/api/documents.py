...
async def _process_and_embed(
    ...
    """Background task: extract text → chunk → embed → persist to ChromaDB."""
    async with AsyncSessionLocal() as db:
        try:
            ...
            logger.error("Document processing error for %s: %s", doc_id, exc)
            doc = await db.get(Document, doc_id)
            if doc:
                doc.status = "error"
...
async def upload_document(
    ...
    # Persist document record (status=processing)
    doc = Document(
